...
// TargetAt implements TargetAbstract.
func (x *Targets) TargetAt(index int) TargetAbstract {
	self := targetAbstract{targetEngine.Abstract(e.TypeID(TargetTypeTargets), e.Ptr(x))}
	return self.TargetAt(index)
}

// TargetCount returns 0.
func (x *Targets) TargetCount() int { return 0 }

// TargetTypeID returns TargetTypeTargets.
func (*Targets) TargetTypeID() TargetTypeID { return TargetTypeTargets }

// WalkTarget visits the receiver with the provided callback.
func (x *Targets) WalkTarget(fn TargetWalkerFn) (_ *Targets, changed bool, err error) {
	var y e.Ptr
	_, y, changed, err = targetEngine.Execute(fn, e.TypeID(TargetTypeTargets), e.Ptr(x), e.TypeID(TargetTypeTargets))
	if err != nil {
		return nil, false, err
	}
	return (*Targets)(y), changed, nil
}
